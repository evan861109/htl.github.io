...
Artist-facing content lives in `content/site.json` and `content/media.json`. The pages read these files at runtime, so the visual design and animation code stay separate from the editable biography, featured videos, media archive, projects, links, and contact details.
...
3. Open this repository in Pages CMS. It will detect `.pages.yml` and show **Artist website** and **Media library** editors.
