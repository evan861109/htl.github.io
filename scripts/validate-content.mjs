...
const validateSite = async (site) => {
  ...
  for (const language of ["en", "zh_hant"]) {
    ...
    const visibleContactCopy = [content.contact?.heading, content.contact?.availability, content.contact?.notice].filter(Boolean).join(" ");
    if (/[^\s@]+@[^\s@]+\.[^\s@]+/.test(visibleContactCopy)) {
      fail(`${prefix} visible contact copy must not display an email address`);
    }
